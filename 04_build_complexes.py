...
# CCDC provenance only; complex-generation logic is unchanged.
CCDC_MAP = {}
if "parent_ccdc" in df_mut.columns:
    for _, r in df_mut.iterrows():
        lig = str(r.get("smiles", "")).strip()
        cc = r.get("parent_ccdc", "")
        if lig and lig not in CCDC_MAP and pd.notna(cc) and str(cc).strip():
            CCDC_MAP[lig] = str(cc).strip()
...
while len(rows) < N_COMPLEXES:
    ...
    rows.append({
        ...
        "donor_sum": TARGET,
        "parent_ccdcs": ";".join(CCDC_MAP.get(l, "") for l, _ in chosen)
    })
