...
st.write("This app uses a GA + GNN oracle pipeline to generate ligand combinations for a target ZFS value.")
...
if run:
    ...
    # ================= RESUME =================
    try:
        if download_elite_from_drive(target_zfs, mode):
    ...
    for gen in range(1, int(max_gen) + 1):
        ...
        if gen == 1 and not os.path.exists("elite_parents.csv"):
            ...
            subprocess.call([PYTHON, "00_build_ligand_donor_map.py"], cwd=WORKDIR)

            st.write("Selecting seed complexes")
            subprocess.call([PYTHON, "01_select_seeds.py"], cwd=WORKDIR)

            st.write("Extracting seed ligands")
            subprocess.call([PYTHON, "02_extract_seed_ligands.py"], cwd=WORKDIR)

        # ================= GA CORE =================
        subprocess.call([PYTHON, "03_ligand_mutation.py"], cwd=WORKDIR)
        subprocess.call([PYTHON, "04_build_complexes.py"], cwd=WORKDIR)
        subprocess.call([PYTHON, "05_oracle_screen.py"], cwd=WORKDIR)
        ...
        if os.path.exists("elite_parents.csv"):
            ...
            if not elite.empty:
                ...
                try:
                    ...
                    st.write("☁️ Drive upload successful")
                except Exception as e:
