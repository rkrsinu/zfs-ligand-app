...
from gdrive_save import (
    ...
    upload_elite_to_drive,
    download_elite_from_drive,
)
...
def show_elite():
    ...
    show_cols = [
        c for c in ["FileName", "ligands", "zfs_pred", "ed_pred", "donor_sum"]
        if c in df.columns
    ]
...
if run_button:
    ...
    # ⭐ LOAD ELITE FROM DRIVE IF EXISTS (iteration memory)
    try:
        download_elite_from_drive(target_zfs, mode)
    except Exception:
        st.warning("Drive elite download skipped")
    ...
    for gen in range(1, int(max_generations) + 1):
        ...
        # ⭐ SAFE ELITE UPLOAD (never crashes GA)
        try:
            upload_elite_to_drive(target_zfs, mode)
        except Exception:
            st.warning(f"Drive upload failed in generation {gen}")
